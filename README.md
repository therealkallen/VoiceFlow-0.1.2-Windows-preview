...
Windows x64 early preview. Start with [building from source](docs/build-from-source.md).
VoiceFlow source is [MIT licensed](LICENSE); model weights and dependencies
have [separate licenses](THIRD_PARTY_NOTICES.md). Source releases exclude model
weights, credentials and personal runtime data. Existing local desktop ZIPs
are test artifacts pending completion of binary redistribution notices.

VoiceFlow is a Windows-first speech input method prototype. The Rust
`input-host` is the runtime center: it listens for the shortcut, captures audio,
runs the speech pipeline, routes the session, and inserts text into the active
Windows app.

The Windows desktop entry point is the Tauri application in `apps/desktop`.
It opens Settings in its own window, keeps dictation running in the tray, and
owns the background host processes. See [desktop usage](docs/desktop-usage.md)
for the portable layout, development commands and company-machine checks.
The earlier `VoiceFlow-portable-review` ZIP is a backend validation build;
desktop builds use `VoiceFlow.exe` instead of browser-launching scripts.
...
- Local ASR worker path.
- Provider refinement for Alibaba Cloud Bailian, Volcengine Ark, Tencent
  Hunyuan, and Custom OpenAI-compatible endpoints.
- Model profile overrides.
- Prompt-file overrides.
- Compact bottom-center Wry/WebView2 overlay with listening, thinking, done,
  and error states. It is click-through, non-activating, always on top,
  DPI-aware, and localized in English and Chinese.
- Localhost-served Overview, History, Settings, and diagnostics companion with
  English/Chinese localization.
- Persisted `system_language` and `ui_style` settings.
- Seven-day Overview analytics backed by usage-ledger schema v2, with
  immediate per-session persistence.
- Local history ledger.
- Configurable local history retention in History & Privacy settings.
- Clear History from the localhost Settings UI.
- Editable non-secret provider settings, secure per-provider API-key storage in
  Windows Credential Manager, and provider Test Connection.
- Five-section Settings navigation with accurate dirty/save state and live
  runtime settings reload.
- Privacy-safe selected-text history.
- Selected-text edit and freeform provider-backed selected-text edit.
- Instructed Dictation with explicit trigger phrase.
- Fast selected-text probing on ordinary push-to-talk sessions, without the old
  multi-second strict no-selection timeout.
- Clipboard insertion for multiline text, with clipboard backup/restore.
  If multiline paste fails, insertion fails without synthesizing Enter keys.
- Short-clip outer-silence trimming before whole-clip ASR.
- Length-aware normal dictation: up to 15 text units uses local cleanup,
  16–60 uses light provider cleanup, and over 60 uses structured cleanup.
  Units count Chinese characters, English words and number groups; detected
  self-corrections use the light provider route even below the local threshold.
- Reused HTTP agents for compatible provider calls.
- Persistent SenseVoice worker preparation with a non-user-visible 250 ms
  synthetic-silence inference warm-up. Warm-up failure is nonfatal and never
  sends a cloud request.
- Safe CPAL host/default-device probing before live-host readiness without
  opening an input stream or starting microphone capture.
- End-to-end ASR, refinement, provider, insertion, selected-text probe, and
...
See [portable build and usage](docs/portable-usage.md). `scripts/build-portable.ps1`
